...
The app currently ships with a realistic seed dataset (21 shops around Philadelphia, PA) so it runs with zero configuration. The UI talks only to the `ShopDataAdapter` interface in `src/data/adapters.ts`; wiring in live data is a drop-in change behind a small backend:
